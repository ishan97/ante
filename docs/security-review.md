...
| Paste | No confirmation dialog (removed in 1.3: it got in the way of answering agents). SwiftTerm wraps a paste in bracketed-paste markers whenever the foreground program has enabled mode 2004 (zsh ≥ 5.1, bash ≥ 4.4, fish and the agents do), so it arrives as one block; a program that has not enabled it receives the lines as typed. | accepted risk (1.3) |
