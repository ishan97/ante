...
|---|---|---|
| No network access | `git grep -nE "URLSession\|NWConnection\|CFStream\|NSURLConnection\|socket\("` over `Packages` and `Ante` → no matches. Running app: `lsof -a -p <pid> -i` → no sockets; `nettop -p <pid> -L 1` → no rows. | pass |
...
| Paste safety | `AnteTerminalView.paste` shows a sheet with a line count and a preview before pasting text that contains a newline (`[security] confirm_multiline_paste`, default on). Bracketed paste is SwiftTerm's default. | pass |
